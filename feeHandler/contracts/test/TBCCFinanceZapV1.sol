...
import {ITBCCFinancePair} from "../../lib/ITBCCFinancePair.sol";
import {ITBCCFinanceRouter} from "../../lib/ITBCCFinanceRouter.sol";
import {IWETH} from "../../lib/IWETH.sol";
import {Babylonian} from "./Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by TBCCFinance teams.
 */
contract TBCCFinanceZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped BNB (WBNB)
    IWETH public WBNB;

    // TBCCFinanceRouter interface
    ITBCCFinanceRouter public tbccFinanceRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from TBCCFinance)
    uint256 public constant MINIMUM_AMOUNT = 1000;

    // Maximum reverse zap ratio (100 --> 1%, 1000 --> 0.1%)
    uint256 public maxZapReverseRatio;

    // Address TBCCFinanceRouter
    address private tbccFinanceRouterAddress;
     ...
     * @param _tbccFinanceRouter: address of the TBCCFinanceRouter
     * @param _maxZapReverseRatio: maximum zap ratio
     */
    constructor(
        address _WBNBAddress,
        address _tbccFinanceRouter,
        uint256 _maxZapReverseRatio
    ) {
        WBNBAddress = _WBNBAddress;
        WBNB = IWETH(_WBNBAddress);
        tbccFinanceRouterAddress = _tbccFinanceRouter;
        tbccFinanceRouter = ITBCCFinanceRouter(_tbccFinanceRouter);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /*
     * @notice Zap BNB in a WBNB pool (e.g. WBNB/token)
     * @param _lpToken: LP token address (e.g. TBCC/BNB)
     * @param _tokenAmountOutMin: minimum token amount (e.g. TBCC) to receive in the intermediary swap (e.g. BNB --> TBCC)
     */
     ...
     * @param _lpToken: LP token address (e.g. TBCC/BUSD)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. TBCC) in the intermediary swap (e.g. BUSD --> TBCC)
     */
    function zapInToken(
        address _tokenToZap,
     ...
     * @param _lpToken: LP token address (e.g. TBCC/WBNB)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in BNB/WBNB) in the intermediary swap (e.g. TBCC --> BNB)
     */
    function zapOutBNB(
        address _lpToken,
     ...
     * @param _lpToken: LP token address (e.g. TBCC/BUSD)
     * @param _tokenToReceive: one of the 2 tokens from the LP (e.g. TBCC or BUSD)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum token to receive (e.g. TBCC) in the intermediary swap (e.g. BUSD --> TBCC)
     */
    ...
    {
        address token0 = ITBCCFinancePair(_lpToken).token0();
        address token1 = ITBCCFinancePair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = ITBCCFinancePair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = tbccFinanceRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = tbccFinanceRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == ITBCCFinancePair(_lpToken).token0() || _token0ToZap == ITBCCFinancePair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == ITBCCFinancePair(_lpToken).token0() || _token1ToZap == ITBCCFinancePair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == ITBCCFinancePair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / ITBCCFinancePair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / ITBCCFinancePair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = tbccFinanceRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = tbccFinanceRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = ITBCCFinancePair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "TBCCFinanceRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "TBCCFinanceRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        tbccFinanceRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (IERC20(_token).allowance(address(this), tbccFinanceRouterAddress) < 1e24) {
            // Re-approve
            IERC20(_token).safeApprove(tbccFinanceRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = tbccFinanceRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = tbccFinanceRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = tbccFinanceRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = tbccFinanceRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = tbccFinanceRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
